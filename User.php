...
class User {
	...
	function checkUser($userData = array()){
        if(!empty($userData)){
			...
			$prevQuery1 = "SELECT * FROM users WHERE first_name='".$userData['first_name']."'";
            $prevResult1 = $this->db->query($prevQuery1);
			 if($prevResult1->num_rows > 0){
                //Update user data if already exists
                $query1 = "update users set first_name='".$userData['first_name']."', last_name='".$userData['last_name']."',email='".$userData['email']."', gender='".$userData['gender']."',locale='".$userData['locale']."', picture='".$userData['picture']."',link='".$userData['link']."', modified='".date("Y-m-d H:i:s")."' WHERE oauth_provider = '".$userData['oauth_provider']."' AND oauth_uid = '".$userData['oauth_uid']."'";
                $update1 = $this->db->query($query1);
            }else{
                //Insert user data
                $user_activation_code = md5(rand());
                $user_email_status= 'not verified';
                $query1 = "INSERT INTO users (oauth_provider,oauth_uid,first_name,last_name,email,gender,locale,picture,link,created,modified) VALUES ('".$userData['oauth_provider']."','".$userData['oauth_uid']."','".$userData['first_name']."','".$userData['last_name']."','".$userData['email']."','".$userData['gender']."','".$userData['locale']."','".$userData['picture']."','".$userData['link']."','".date("Y-m-d H:i:s")."','".date("Y-m-d H:i:s")."')";
